...
--DROP TABLE M_UDC_MAP;
CREATE TABLE M_UDC_MAP (
  PROVINCE nchar(10) not null,
  CITY nchar(10) not null,
  PREFECTURE nchar(10) not null,
  DISTRICT nchar(40) not null,
  CONSTRAINT M_UDC_MAP_PK PRIMARY KEY (PROVINCE,CITY,PREFECTURE,DISTRICT)
) NOLOGGING;
--INSERT INTO M_UDC_MAP SELECT * FROM S_M_UDC_MAP;

COMMENT ON TABLE M_UDC_MAP IS 'UDC(行政区域)';
COMMENT ON COLUMN M_UDC_MAP.PROVINCE IS '省';
COMMENT ON COLUMN M_UDC_MAP.CITY IS '市';
COMMENT ON COLUMN M_UDC_MAP.PREFECTURE IS '县／镇／区';
COMMENT ON COLUMN M_UDC_MAP.DISTRICT IS '区／村／街道';
...
COMMENT ON COLUMN M_CO.CUR_CODE IS '货币代码';
...
CREATE TABLE M_BP_MCU (
  ...
  CONSTRAINT M_BP_MCU_PK PRIMARY KEY (CODE)
) NOLOGGING;
...
COMMENT ON COLUMN M_ITEM.LINE_TYPE IS '？？？';
COMMENT ON COLUMN M_ITEM.INV_STATUS IS '库存状态（O: 已下架）';
...
COMMENT ON COLUMN M_ITEM.CODE_OLD IS '原来的产品号';
...
--DROP TABLE M_UOM_EA2KG;
CREATE TABLE M_UOM_EA2KG (
  JD number(6) DEFAULT null,
  ITEM_NO number(8) not null,
  KG_D7 number(18) DEFAULT null,
  CONSTRAINT M_UOM_EA2KG_PK PRIMARY KEY (ITEM_NO)
) NOLOGGING;
--INSERT INTO M_UOM_EA2KG SELECT * FROM S_M_UOM_EA2KG;

COMMENT ON TABLE M_UOM_EA2KG IS 'UOM重量';
COMMENT ON COLUMN M_UOM_EA2KG.ITEM_NO IS '产品号';
COMMENT ON COLUMN M_UOM_EA2KG.KG_D7 IS '主计量单位的重量（单位：KG/10^7）';


--DROP TABLE M_UOM_XX2EA;
CREATE TABLE M_UOM_XX2EA (
  JD number(6) DEFAULT null,
  ITEM_NO number(8) not null,
  XX nchar(2) not null,
  EA_M7 number(18) DEFAULT null,
  CONSTRAINT M_UOM_XX2EA_PK PRIMARY KEY (ITEM_NO,XX)
) NOLOGGING;
--INSERT INTO M_UOM_XX2EA SELECT * FROM S_M_UOM_XX2EA;

COMMENT ON TABLE M_UOM_XX2EA IS '各种计量单位到UOM的数量转换矩阵';
COMMENT ON COLUMN M_UOM_XX2EA.ITEM_NO IS '产品号';
COMMENT ON COLUMN M_UOM_XX2EA.XX IS '计量单位（CA/BX）';
COMMENT ON COLUMN M_UOM_XX2EA.EA_M7 IS '转换UOM的放大系数';
